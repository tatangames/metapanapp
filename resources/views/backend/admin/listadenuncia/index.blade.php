                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group" >
                                        <label class="control-label">Tipo Denuncia</label>
                                        <select id="select-denuncia" class="form-control">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group" >
                                        <label class="control-label">Tipo Denuncia</label>
                                        <select id="select-denuncia-editar" class="form-control">
                                        </select>
                                    </div>

                                    <div class="form-group" >
                                        <label class="control-label">Departamento</label>
                                        <select id="select-departamento-editar" class="form-control">
                                        </select>
                                    </div>
...
@section('archivos-js')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            var ruta = "{{ URL::to('/admin/listadenuncia/listado-tabla') }}";
    ...
    <script>
        ...
        function nuevo(){
            ...
            var tipodenuncia = document.getElementById('select-denuncia').value;
            ...
            if(descripcion.length > 100){
                toastMensaje('error', '100 caracteres máximo para Descripción');
                return;
            }
            ...
            axios.post('/admin/listadenuncia/listado-nuevo', formData, {
            })
                .then((response) => {
                    closeLoading()
        ...
        function recargar(){
            var ruta = "{{ url('/admin/listadenuncia/listado-tabla') }}/";
            $('#tablaDatatable').load(ruta);
        }
        ...
        function modalInformacion(id){
            ...
            axios.post('/admin/listadenuncia/listado-info',{
                'id': id
            })
                .then((response) => {
                    closeLoading();
                    if(response.data.success == 1){
                        ...
                        document.getElementById("select-denuncia-editar").options.length = 0;
                        document.getElementById("select-departamento-editar").options.length = 0;

                        $.each(response.data.tipo, function( key, val ){
                            if(response.data.info.id_tipo_envio_denuncia == val.id){
                                $('#select-denuncia-editar').append('<option value="' +val.id +'" selected="selected">'+val.nombre+'</option>');
                            }else{
                                $('#select-denuncia-editar').append('<option value="' +val.id +'">'+val.nombre+'</option>');
                            }
                        });

                        $.each(response.data.tipo2, function( key, val ){
                            if(response.data.info.id_departamento_denuncia == val.id){
        ...
        function editar(){
            ...
            var tipodenuncia = document.getElementById('select-denuncia-editar').value;
            ...
            axios.post('/admin/listadenuncia/listado-editar', formData, {
            })
                .then((response) => {
                    closeLoading();
                    if(response.data.success === 1){
        ...
        // vista para agregar multiples opciones a la denuncia
        function denunciaMultiple(id){
            window.location.href="{{ url('/admin/listadenuncia/opciones-multiples') }}/"+id;
        }
